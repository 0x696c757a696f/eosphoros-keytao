...
class EosphorosCodeTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        from tools.eosphoros_codes import load_character_codes

        cls.character_codes = load_character_codes(
            ROOT / "dicts" / "eosphoros" / "eosphoros.danzi.dict.yaml", {"召": "fz"}
        )

    def test_generates_standard_two_three_and_multi_character_codes(self) -> None:
        from tools.eosphoros_codes import code_candidates
    ...
    def test_uses_first_character_auxiliary_codes_to_avoid_collision(self) -> None:
        from tools.eosphoros_codes import choose_code
