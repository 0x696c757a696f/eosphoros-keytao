...
local config = require("eosphoros.common.eosphoros_config")
local platform = require("eosphoros.common.eosphoros_platform")
local candidate = require("eosphoros.common.eosphoros_candidate")
...
function M.wrap_append_if_needed(cand, env, ctx, input_text, first)
    ...
    if cand and cand._eosphoros_zzc_preedit_only then return cand end
    local source_input = ctx:get_property(M.append_input_key(env))
    local suffix = ctx:get_property(M.append_suffix_key(env))
