...
local function module_project_dir()
    ...
    source = source:gsub("\\", "/")
    local lua_module_dir = dirname(source)
    if lua_module_dir:match("/lua/eosphoros$") then
        return dirname(dirname(lua_module_dir))
    end
    if lua_module_dir:match("/lua$") or lua_module_dir == "lua" then
        return dirname(lua_module_dir)
    end
    return lua_module_dir
end
...
function M.init(env)
    ...
    if not dict_file or dict_file == "" then
        dict_file = (env._cx_dict or "eosphoros.cx") .. ".dict.yaml"
    end
