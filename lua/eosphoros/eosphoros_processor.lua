...
local direct_symbols = require("eosphoros.input.eosphoros_direct_symbols")
local ascii_input = require("eosphoros.input.eosphoros_ascii_input")
local topup = require("eosphoros.input.eosphoros_topup")
...
local function processor(key_event, env)
    ...
    local english_prefix = env._english_prefix
    local current_input = ctx.input or ""
    if is_code_key and type(english_prefix) == "string" and english_prefix ~= ""
        and string_sub(current_input, 1, #english_prefix) == english_prefix then
        _space_guard_clear(env)
        return kNoop
    end
    ...
    if not env._tu_streaming and is_code_key then
        ...
        if not eval.semicolon_input and not (env._tu_cmd and is_ftu) then
            if is_ptu and not is_tu then
                env._eosphoros_zzc_follow_key = key
                local executed, consumed_follow, transitioned = _topup_exec(env)
                env._eosphoros_zzc_follow_key = nil
...
local function init(env)
    ...
    env._space_guard_enabled = config:get_string("eosphoros/space_guard") ~= "off"
    env._direct_symbols_fast_leaf = _config_bool(config, "eosphoros/direct_symbols_fast_leaf", true)
