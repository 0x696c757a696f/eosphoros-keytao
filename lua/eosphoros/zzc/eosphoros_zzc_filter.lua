...
local processor = require("eosphoros.zzc.eosphoros_zzc_processor")
local length_inputs = require("eosphoros.zzc.eosphoros_zzc_keys").length_keys
...
local function literal_length_input(ctx, input_text)
    ...
    if ctx:get_property("_eosphoros_zzc_stage") ~= "collect" or ctx:get_property("_eosphoros_zzc_mode") ~= "make" then return nil end
    local word = ctx:get_property("_eosphoros_zzc_word") or ""
...
local function state_candidate(ctx, code, env)
    local prop_stage = ctx and ctx.get_property and ctx:get_property("_eosphoros_zzc_stage") or ""
    local prop_word = ctx and ctx.get_property and ctx:get_property("_eosphoros_zzc_word") or ""
    local prop_mode = ctx and ctx.get_property and ctx:get_property("_eosphoros_zzc_mode") or ""
    local prop_display = ctx and ctx.get_property and ctx:get_property("_eosphoros_zzc_display") or ""
    local prop_target = ctx and ctx.get_property and ctx:get_property("_eosphoros_zzc_target") or ""
    local prop_items = ctx and ctx.get_property and ctx:get_property("_eosphoros_zzc_items") or ""
...
local function collect_make_preedit(ctx, code)
    ...
    local word = core.buffer_word() or (ctx and ctx.get_property and ctx:get_property("_eosphoros_zzc_word") or "")
    if word == "" then return nil end
...
local function yield_restore_candidates(ctx, code)
    ...
    local rows_text = ctx and ctx.get_property and ctx:get_property("_eosphoros_zzc_cmd_candidates") or ""
    local idx = 0
    local yielded = false
...
local function yield_code_choice_candidates(ctx, code)
    ...
    local len = tonumber(ctx and ctx.get_property and ctx:get_property("_eosphoros_zzc_len") or "")
    for line in rows_text:gmatch("[^\n]+") do
...
local function with_preedit(cand, preedit_text)
    ...
    if not ok or not nc then
        ...
        cand._eosphoros_zzc_preedit_only = true
        return cand
    end
    nc.preedit = preedit_text
    nc.quality = cand.quality
    nc._eosphoros_zzc_preedit_only = true
    return nc
end
...
local function filter(input, env)
    ...
    if literal_len and processor.finalize_literal_length(ctx, env, literal_len) then
        ...
        prop_stage = ctx and ctx.get_property and ctx:get_property("_eosphoros_zzc_stage") or ""
        prop_mode = ctx and ctx.get_property and ctx:get_property("_eosphoros_zzc_mode") or ""
