...
local state_model = require("eosphoros.zzc.eosphoros_zzc_state")
local keys = require("eosphoros.zzc.eosphoros_zzc_keys")
local candidates = require("eosphoros.zzc.eosphoros_zzc_candidates")
...
local function show_selected_code_notice(ctx, word, code)
    ...
    ctx:set_property("_eosphoros_zzc_stage", "resolve_notice")
    ctx:set_property("_eosphoros_zzc_word", word or "")
    ctx:set_property("_eosphoros_zzc_target", code or "")
    ctx:set_property("_eosphoros_zzc_display", word or "")
end
...
local function recover_collect_items(ctx)
    ...
    if ctx and ctx.get_property then
        local prop_items = ctx:get_property("_eosphoros_zzc_items") or ""
        ...
        local prop_stage = ctx:get_property("_eosphoros_zzc_stage") or ""
        local prop_mode = ctx:get_property("_eosphoros_zzc_mode") or ""
        local prop_target = ctx:get_property("_eosphoros_zzc_target") or ""
        local prop_display = ctx:get_property("_eosphoros_zzc_display") or ""
        local prop_word = ctx:get_property("_eosphoros_zzc_word") or ""
...
local function handle_collect_backspace(ctx)
    if state.stage == "collect" then
        ...
        if (not state.display_word or state.display_word == "") and ctx and ctx.get_property then
            state.display_word = core.buffer_word() or (ctx:get_property("_eosphoros_zzc_word") or "")
        end
    end
...
local function finalize_with_length(ctx, len, env)
    if not len then return kAccepted end
    if ctx and ctx.set_property then ctx:set_property("_eosphoros_zzc_len", tostring(len)) end
...
local function processor(key_event, env)
    ...
    if current_input == "" and code_char and composition_empty(ctx) then
        local prop_stage = ctx and ctx.get_property and (ctx:get_property("_eosphoros_zzc_stage") or "") or ""
...
local function module_is_active(ctx)
    ...
    if ctx and ctx.get_property then
        return (ctx:get_property("_eosphoros_zzc_stage") or "") ~= ""
    end
    return false
end
