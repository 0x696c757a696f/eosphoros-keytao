...
local M = {}
local codec = require("eosphoros.zzc.eosphoros_zzc_codec")
local chain = require("eosphoros.zzc.eosphoros_zzc_chain")
local store = require("eosphoros.zzc.eosphoros_zzc_store")
...
local function load_char_parts_from_dict(target)
    local f = io.open(path("dicts/eosphoros/eosphoros.danzi.dict.yaml"), "r")
...
local function ops_file()
    return path("dicts/eosphoros/eosphoros.zzc.dict.yaml")
end
...
local function legacy_root_ops_file()
    return path("eosphoros.zzc.ops.tsv")
end
...
local function reset_marker_from_file(file_path)
    ...
    f:close()
    if marker.version ~= "2" or marker.schema ~= "eosphoros" or marker.mode ~= "full_reset" then
        return nil
    end
