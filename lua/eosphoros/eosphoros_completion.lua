...
local function core_dict_candidates(schema_id)
    local file_name = ((schema_id and schema_id ~= "") and schema_id or "eosphoros") .. ".core.dict.yaml"
...
return {
    init = function(env)
        ...
        env._completion_schema_id = env.engine.schema.schema_id or "eosphoros"
        env._danzi_first = not (config:get_bool("translator/enable_sentence") or false)
        env._max_scan_candidates = config:get_int("eosphoros_completion/max_scan_candidates") or DEFAULT_MAX_SCAN_CANDIDATES
        env._max_deferred_candidates = config:get_int("eosphoros_completion/max_deferred_candidates") or DEFAULT_MAX_DEFERRED_CANDIDATES
    ...
    func = function(input, env)
        ...
        local zzc_stage = ctx and ctx.get_property and (ctx:get_property("_eosphoros_zzc_stage") or "") or ""
        local zzc_mode = ctx and ctx.get_property and (ctx:get_property("_eosphoros_zzc_mode") or "") or ""
        local zzc_target = ctx and ctx.get_property and (ctx:get_property("_eosphoros_zzc_target") or "") or ""
        ...
        for cand in input:iter() do
            ...
            if not danzi then
                ...
                if c and type(c) == "string" and #c == 0 then
                    ...
                    if text_len == 1 then
                        yield(cand)
                    elseif text_len and text_len > 1
                        and (max_deferred_candidates == 0 or buffer_size < max_deferred_candidates) then
