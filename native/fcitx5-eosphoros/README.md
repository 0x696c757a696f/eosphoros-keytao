...
在空的 ZZC 会话输入 `--` 后按反斜杠可撤回最近一次自造词，输入 `!!!` 后按反斜杠
可清空自造词；选中自造词候选时按 `Ctrl+Delete` 可删除该条。学习和自造词直接保存
在原生 TSV 中，不需要再运行 Rime 版合并脚本。

本目录构建的是 Fcitx5 输入法 addon，不是可单独导入的普通码表。macOS 和 Android
用户继续使用仓库对应平台的 Rime 方案包与 Fcitx5 官方 Rime 插件即可，不需要安装
本 addon；Linux 用户也可以选择该方式，原生 addon 只是无 Rime 路线的可选版本。
