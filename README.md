...
| --- | ---: | ---: | ---: | --- |
| Full | 1,175,788 | 57,153 | 4.861% | Lite + 宗派专题 + ICE + 万象专业 + `fjcy` |
| Standard | 661,777 | 21,460 | 3.243% | Lite + 宗派专题 + ICE + 万象专业 |
| Lite | 251,492 | 12,031 | 4.784% | 用户词库 + 单字 + 基础词组 + 核心码 + `i` 英文 |
...
| 词库 | 记录数 | 用途 |
| --- | ---: | --- |
| `dicts/eosphoros/eosphoros.danzi.dict.yaml` | 36,292 | 上游键道单字表 |
| `dicts/eosphoros/eosphoros.cizu.dict.yaml` | 190,651 | 本地基础词组 |
...
| `dicts/eosphoros/eosphoros.ice.dict.yaml` | 360,435 | Rime-Ice 中文精简补充词库 |
| `dicts/eosphoros/eosphoros.wanxiang.*.dict.yaml` | 45,949 | 七个万象分类补充词库 |
| `dicts/eosphoros/eosphoros.en.dict.yaml` | 23,648 | Rime-Ice 英文词库 |
| **合计** | **1,175,788** | 不含动态自造词和个人词库 |
...
`pixi run check` 会先运行全部回归测试，再将生成文件与锁定的 SHA-256 结果逐一比对。
上游更新时运行 `pixi run generated` 重算并更新对应哈希；无关的本地元数据改动不会阻断词库检查。
