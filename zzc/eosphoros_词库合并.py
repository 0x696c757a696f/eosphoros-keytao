#!/usr/bin/env python3
"""Platform-neutral eosphoros ZZZC dictionary merge core."""
    ...
    for options in target_dict_name_options(prefix):
        ...
        for name in options:
            ...
            if path.exists():
                matched = path
                break
        out.append(matched or (dictionary_root / options[0]))
    return out
...
def discover_layout() -> tuple[Path, Path, Path, tuple[Path, ...], str]:
    root_override = os.environ.get("eosphoros_zzc_root")
    if root_override:
        ...
        ops_search_dirs = [start_dir, start_dir / "dicts" / "eosphoros", start_dir / "zzc"]
    else:
        ...
        ops_search_dirs = search_dirs + [base / "dicts" / "eosphoros" for base in search_dirs]
...
STATE_DIR = (
    Path(os.environ["eosphoros_zzc_state_dir"]).expanduser().resolve()
    if os.environ.get("eosphoros_zzc_state_dir")
    else ROOT / "zzc_state"
)
...
CHAR_DICT = ROOT / "dicts" / "eosphoros" / f"{SCHEMA}.danzi.dict.yaml"
