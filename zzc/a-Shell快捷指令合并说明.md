# a-Shell 免费方案：iOS 快捷指令合并 eosphoros 自造词

适用于 iOS 16 或更新版本、a-Shell 和当前 `iOS_词库合并.py`。

## 准备文件

从 App Store 安装 a-Shell，并把下面两个文件放到 a-Shell 可以访问的同一目录，例如 `~/Documents/eosphoros-zzc/`：
...
- 使用 iCloud：选择 iCloud 中的 `RimeUserData` 或其中的 eosphoros 方案目录。
- 不使用 iCloud：选择输入法应用文件中的 `RimeUserData` 或 eosphoros 方案目录。
...
```sh
cd ~/Documents/eosphoros-zzc && python3 iOS_词库合并.py; open shortcuts://
```

若没有 a-Shell 原生动作，可用“打开 URL”运行：

```text
a-shell://?command=cd%20~/Documents/eosphoros-zzc%20%26%26%20python3%20iOS_%E8%AF%8D%E5%BA%93%E5%90%88%E5%B9%B6.py
```
